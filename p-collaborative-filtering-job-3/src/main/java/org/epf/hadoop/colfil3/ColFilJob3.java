...
public class ColFilJob3 extends Configured implements Tool {
    ...
    public int run(String[] args) throws Exception {
        ...
        Job job = Job.getInstance(conf, "Job3: Friend Recommandations");
        job.setJarByClass(getClass());

        job.setInputFormatClass(TextInputFormat.class);

        job.setMapperClass(RecommandationMapper.class);
        job.setReducerClass(RecommandationReducer.class);

        job.setMapOutputKeyClass(Text.class);
        job.setMapOutputValueClass(RelationRecommandation.class);
